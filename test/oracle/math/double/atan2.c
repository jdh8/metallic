    ...
    for (uint64_t k = 0; k < n; ++k) {
        ...
        if (shown < 10) {
            ...
            fprintf(stderr, "  y=%a x=%a  got=%a  want=%a  (%lld ulp)\n",
                y, x, fut2(y, x), ref2(y, x), (long long)d);
        }
    }

    fprintf(stderr, "  random[2^%d,2^%d]^2 x%llu: %llu bad, worst %lld ulp at y=%a x=%a\n",
        emin, emax, (unsigned long long)n, (unsigned long long)bad,
        (long long)worst, worst_y, worst_x);
    return bad;
}

/* Test every pair in a CORE-MATH bivariate `*.wc` file for binary64.
 * Lines have the form "y,x" (same order as atan2 args); snan lines are skipped. */
                                 ...
                                 const char *path)
{
    ...
    fclose(f);
    fprintf(stderr, "  worst-cases %s (%llu pairs): %llu bad, worst %lld ulp at y=%a x=%a\n",
