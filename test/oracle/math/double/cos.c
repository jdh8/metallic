...
#include "binary64/cos/cos_mpfr.c"
#include "../../sample.h"

static double fut(double x) { return cos(x); }
static double ref(double x) { return ref_cos(x); }

int main(void)
{
    uint64_t bad = 0;
    bad |= sample_wc_f64(fut, ref, CORE_MATH "/binary64/cos/cos.wc");
