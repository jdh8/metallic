...
#include "binary64/erfc/erfc_mpfr.c"
#include "../../sample.h"

static double fut(double x) { return erfc(x); }
static double ref(double x) { return ref_erfc(x); }
...
int main(void)
{
    ...
    ref_init();
    bad |= sample_wc_f64(fut, ref, CORE_MATH "/binary64/erfc/erfc.wc");
