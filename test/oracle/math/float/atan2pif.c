...
#include "binary32/atan2pi/atan2pif_mpfr.c"
#include "../../sample.h"

static float fut(float y, float x) { return atan2pif(y, x); }
static float ref(float y, float x) { return ref_atan2pi(y, x); }
...
int main(void)
{
    ...
    ref_init();
    bad |= sample_wc_f32_2(fut, ref, CORE_MATH "/binary32/atan2pi/atan2pif.wc");
